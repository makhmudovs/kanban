...
export function SortBar({
  ...
  const options: SortOption[] = ["Most Upvotes", "Least Upvotes", "Most Comments", "Least Comments"];
  ...
  return (
    <div className="bg-[#373F68] rounded-none sm:rounded-2xl px-6 py-3.5 flex items-center gap-4">
      <div className="hidden sm:flex items-center gap-3 text-white/75 mr-auto">
        ...
        <span className="font-bold text-white text-[15px]">{count} Suggestions</span>
      </div>
      ...
      <button
        onClick={onAdd}
        className="bg-[#AD1FEA] hover:bg-[#C75AF6] transition-colors text-white text-[13px] font-bold px-4 py-2.5 rounded-xl whitespace-nowrap"
      >
        + Add Feedback
      </button>
    </div>
